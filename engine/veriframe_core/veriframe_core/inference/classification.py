...
from veriframe_core.models.output_parsers import parse_classification_output
from veriframe_core.preprocessing.transforms import TransformConfig, build_inference_transform


class ClassificationInferenceRunner(InferenceRunner):
    def predict(self, image_path: str, *, confidence_threshold: float = 0.5):
        ...
        transform = build_inference_transform(
            TransformConfig(
                ...
                center_crop=self.loaded_model.profile.preprocessing.centerCrop,
                normalize=self.loaded_model.profile.preprocessing.normalize,
            )
        )
        tensor = transform(loaded_image.image).unsqueeze(0).to(self.loaded_model.device)

        with torch.inference_mode():
            output = self.loaded_model.model(tensor)
